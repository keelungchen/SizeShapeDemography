...
pa_data_done_2 <- filter(pa_data_done, Survey_times == 2) %>%
  mutate(Fu_ID = str_detect(Source, pattern = "\\+")) %>%
  mutate(Fi_PAST = case_when(Fu_ID == FALSE~1, TRUE~0)) %>%
  mutate(Fu_PAST = case_when(Fu_ID == TRUE~1, TRUE~0)) %>%
  filter(!Fate == "D") %>%
  mutate(Fi_FUT = case_when(Fate == "Fi"~1, TRUE~0)) %>%
  mutate(Fu_FUT = case_when(Fate == "Fu"~1, TRUE~0))
  
model <- glm( Fi_FUT~  Fi_PAST + Fu_PAST + Shape_Area + circ   , data = pa_data_done_2, family = binomial())

model <- glm( Fu_FUT~  Fi_PAST + Fu_PAST + Shape_Area + circ   , data = pa_data_done_2, family = binomial())

str_detect(pa_data_done$Source, pattern = "\\+")

# Clean data
df <- pa_data_done_12 %>% 
  dplyr::select(Discrete_change, Shape_Area, pa_ratio, circ_logit, Unique_ID, Survey_times) %>% 
  mutate(Shape_Area = log10(Shape_Area), pa_ratio = log10(pa_ratio))
...
model <- glmer(df[[1]] ~ df[[i+1]] + (1 | df[[6]]/df[[5]]), family = binomial) #df[5] = unique_ID
model <- glmer(Discrete_change ~ Shape_Area_log + (1 | Survey_times/Unique_ID), data = df, family = binomial)


for (i in 1:3) {
  model <- glmer(df[[1]] ~ df[[i+1]] + (1|df[[6]]) + (1 | df[[5]]:df[[6]]), family = binomial) #df[5] = unique_ID
  print(summary(model)) #show model info
}
...
#GLMM for Fi
```{r}
# create consistent ID for Fi colonies
pa_data_done$Fi_ID <- NA
pa_data_done$ID <- as.character(pa_data_done$ID)

pa_data_done <- pa_data_done %>%
  mutate(Fi_ID = case_when(
    !is.na(Source) & !grepl("\\+", Source) ~ Source,  # source 不是 NA 且不含有 '+' 號
    is.na(Source)  ~ ID,          # fill non-fission with ID 
    grepl("\\+", Source) ~ ID,    # fill non-fission with ID
    TRUE ~ Fi_ID                                      # others keep NA (for checking)
  ))

pa_data_done$Fi_ID <- paste(pa_data_done$Plot_ID, pa_data_done$Fi_ID, sep="_")
pa_data_done$Fi_ID <- as.factor(pa_data_done$Fi_ID)
...
for (Fates in Fate_loop) {
  ...
  pa_data_done_12 <- filter(pa_data_done_12, Fate != "D") #Exclude mortality
  # Clean data
  df <- data.frame(pa_data_done_12$Discrete_change, log10(pa_data_done_12$Shape_Area), log10(pa_data_done_12$pa_ratio), pa_data_done_12$circ_logit, pa_data_done_12$Fi_ID) 

  # Calculate 3 morphological classification
for (i in 1:3) {
  model <- glmer(df[[1]] ~ df[[i+1]] + (1 | df[[5]]), family = binomial) #df[5] = Fi_ID
  print(summary(model))
}
